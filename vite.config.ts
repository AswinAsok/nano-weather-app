...
type MiddlewareContainer = Connect.Server | ViteDevServer['middlewares'] | PreviewServer['middlewares'];

function createImageProxy(env: Record<string, string>) {
  const apiKey = env.VITE_GEMINI_API_KEY || '';
  const model = env.VITE_GEMINI_IMAGE_MODEL || 'gemini-2.0-flash-exp-image-generation';
  ...
  const handler: Connect.NextHandleFunction = (req, res, next) => {
    ...
    req.on('end', async () => {
      try {
        ...
        res.statusCode = 500;
        res.end(
          err instanceof Error ? err.message : 'Image generation failed. Check API key and model availability.'
        );
      }
    });
  };
